...
/* 异常调试打点：进入 fault 后直接整屏显示固定颜色，方便快速判断卡死是否来自异常。 */
static void ui_fault_display_mark(uint16 color)
{
    ips200_full(color);
    while(1);
}
...
void HardFault_Handler(void)
{
	...
	//所以大家需要注意这个问题
	/* 绿色表示当前进入 HardFault。 */
	ui_fault_display_mark(RGB565_GREEN);
}
void MemManage_Handler(void)
{
	/* 青色表示当前进入 MemManage 异常。 */
	ui_fault_display_mark(RGB565_CYAN);
}
void BusFault_Handler(void)
{
	/* 黄色表示当前进入 BusFault 异常。 */
	ui_fault_display_mark(RGB565_YELLOW);
}
void UsageFault_Handler(void)
{
	/* 白色表示当前进入 UsageFault 异常。 */
	ui_fault_display_mark(RGB565_WHITE);
}
